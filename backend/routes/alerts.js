  ...
  try {
    ...
    overdueOrders.forEach(order => {
      ...
      alerts.push({
        ...
        message: `Awaiting confirmation for ${days || 2} days`,
        priority: 1,
        color: 'red',
        action: 'confirm',
    ...
    lowStock.forEach(prod => {
      alerts.push({
        ...
        productId: prod.id,
        title: `${prod.name}`,
        message: `Only ${prod.available_qty} ${prod.unit} remaining`,
        priority: 2,
        color: 'orange',
        action: 'update_stock',
    ...
    dispatchPending.forEach(order => {
      ...
      alerts.push({
        ...
        orderId: order.id,
        title: `Order #${order.id} — ${order.customer_name}`,
        message: `Confirmed ${hours || 24} hours ago`,
        priority: 3,
        color: 'blue',
        action: 'dispatch',
        data: { orderId: order.id }
    ...
    newOrders.forEach(order => {
      alerts.push({
        ...
        orderId: order.id,
        title: `Order #${order.id} — ${order.customer_name}`,
        message: `Placed recently — ${order.total_items || 0} items, ₹${order.total_amount || 0}`,
        priority: 4,
        color: 'green',
        action: 'view',
        data: { orderId: order.id }
